...
class Conv2d_BN(torch.nn.Sequential):
    ...
    def forward(self, x):
        # Handle BF16 compatibility for BatchNorm
        x = self.c(x)
        if x.dtype == torch.bfloat16:
            # Temporarily convert to float32 for BatchNorm computation
            x_float = x.float()
            # Ensure BN weights are float32
            if self.bn.weight.dtype != torch.float32:
                self.bn.weight.data = self.bn.weight.data.float()
            if self.bn.bias.dtype != torch.float32:
            ...
            if self.bn.running_var.dtype != torch.float32:
...
class BN_Linear(torch.nn.Sequential):
    ...
    def forward(self, x):
        ...
        if x.dtype == torch.bfloat16:
            ...
            if self.bn.running_var.dtype != torch.float32:
                self.bn.running_var.data = self.bn.running_var.data.float()
            x_bn = self.bn(x_float)
            # Convert result back to BF16
            x_bn = x_bn.to(x.dtype)
        else:
            x_bn = self.bn(x)
        return self.l(x_bn)
...
class LKP(nn.Module):
    ...
    def forward(self, x):
        # Ensure input dtype matches model weights for BF16 compatibility
        original_dtype = x.dtype
        if x.dtype != self.cv1.c.weight.dtype:
            x = x.to(self.cv1.c.weight.dtype)
...
class LSConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.dtype == torch.bfloat16:
            ...
            # Ensure LKP output is float32
            lkp_out = self.lkp(x_float)
            ska_out = self.ska(x_float, lkp_out)
            result = self.bn(ska_out)
            # Convert result back to BF16
            result = result.to(original_dtype)
        else:
            lkp_out = self.lkp(x)
            ska_out = self.ska(x, lkp_out)
            result = self.bn(ska_out)
        return result + x
